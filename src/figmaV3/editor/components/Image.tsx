...
export const ImageDef: ComponentDefinition<ImageProps, StyleBase> = {
  ...
  defaults: {
    props: { src: "", alt: "" },
    styles: { element: { display: "inline-block", width: 120, height: 80, backgroundColor: "#e5e7eb", objectFit: "cover" } },
  },
  propsSchema: [
  ...
  Render: ({ node }) => {
    ...
    // objectFit props → style 우선 반영(있으면)
    if (n.props.objectFit) style.objectFit = n.props.objectFit;

    // ⚠️ 빈 문자열 src는 브라우저 경고 → undefined로 넣어 회피
